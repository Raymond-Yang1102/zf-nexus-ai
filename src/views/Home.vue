    ...
    <main class="pt-16">
      <section class="relative min-h-[60vh] flex items-center justify-center hero-bg-indigo overflow-hidden">
        <div class="relative z-10 text-center px-4">
          <h1 class="text-4xl md:text-6xl font-bold mb-6 brand-gradient-text filter-text-shadow animate-fade-in">{{
            homeData.hero.title }}</h1>
          <p class="text-lg md:text-xl text-gray-600 max-w-2xl mx-auto animate-fade-in delay-200">{{
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div v-for="(card, key) in homeData.fourCards" :key="key" @click="goToPage(key)"
              class="group p-8 rounded-2xl bg-gray-50 border border-gray-100 hover:border-blue-200 hover:shadow-lg transition-all duration-300 cursor-pointer text-center">
              <div
                class="w-14 h-14 rounded-xl bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center mb-6 mx-auto group-hover:scale-110 transition-transform">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div v-for="(product, key) in homeData.products" :key="key" @click="goToProduct(key)"
              ...
              <div
                class="w-20 h-20 rounded-xl bg-gradient-to-br flex items-center justify-center mb-6 mx-auto group-hover:scale-110 transition-transform"
