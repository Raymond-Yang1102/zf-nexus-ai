<template>
  <div class="min-h-screen bg-white">
    ...
    <main class="pt-16">
      <section class="relative min-h-[60vh] flex items-center justify-center hero-bg-indigo overflow-hidden">
        <div class="relative z-10 text-center px-4">
          <h1 class="text-4xl md:text-6xl font-bold mb-6 brand-gradient-text filter-text-shadow animate-fade-in">{{ homeData.hero.title }}</h1>
          <p class="text-lg md:text-xl text-gray-600 max-w-2xl mx-auto animate-fade-in delay-200">{{ homeData.hero.subtitle }}</p>
        </div>
      </section>
      ...
      <section class="py-16 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div v-for="(card, key) in homeData.fourCards" :key="key"
              @click="goToPage(key)"
      ...
      <section class="py-16 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p class="text-center text-gray-600 mb-8 max-w-3xl mx-auto">{{ homeData.clients.description }}</p>
          <div class="relative overflow-hidden py-8" @mouseenter="stopLogoScroll" @mouseleave="startLogoScroll">
            <div ref="logoScrollContainer" class="flex animate-scroll">
              <div v-for="logo in baseLogos" :key="logo.id" class="flex-shrink-0 px-6 py-4">
                <img :src="logo.src" :alt="logo.alt" class="h-10 md:h-14 w-auto opacity-70 hover:opacity-100 transition-all duration-300" />
              </div>
              <div v-for="logo in baseLogos" :key="`${logo.id}-dup`" class="flex-shrink-0 px-6 py-4">
                <img :src="logo.src" :alt="logo.alt" class="h-10 md:h-14 w-auto opacity-70 hover:opacity-100 transition-all duration-300" />
              </div>
            </div>
            <div class="absolute left-0 top-0 bottom-0 w-24 bg-gradient-to-r from-gray-50 to-transparent pointer-events-none"></div>
            <div class="absolute right-0 top-0 bottom-0 w-24 bg-gradient-to-l from-gray-50 to-transparent pointer-events-none"></div>
          </div>
        </div>
      </section>
      ...
      <section class="py-16 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div v-for="(product, key) in homeData.products" :key="key"
              @click="goToProduct(key)"
              ...
              <div class="w-20 h-20 rounded-xl bg-gradient-to-br flex items-center justify-center mb-6 mx-auto group-hover:scale-110 transition-transform"
                :class="productBgColors[key as keyof typeof productBgColors]">
                <img :src="productLogos[key as keyof typeof productLogos]" :alt="product.name" class="w-10 h-10 object-contain" />
              </div>
              <h3 class="text-xl font-bold text-gray-800 mb-3 text-center">{{ product.name }}</h3>
              <p class="text-gray-600 text-sm leading-relaxed text-center">{{ product.description }}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="py-16 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 class="text-2xl font-bold text-gray-800 text-center mb-8">{{ homeData.news.title }}</h2>
