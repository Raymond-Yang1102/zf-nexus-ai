<template>
  <div class="min-h-screen bg-white"><Header :currentLocale="currentLocale" @change-locale="changeLocale"/><main class="pt-16">
    ...
    <section class="py-16 bg-white"><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div class="flex flex-col lg:flex-row gap-12 items-center"><div class="lg:w-1/2"><h2 class="text-2xl font-bold text-gray-800 mb-8">{{ technology.workflowTitle }}</h2><div class="space-y-4 text-gray-600 leading-relaxed"><p v-for="item in technology.workflow" :key="item">{{ item }}</p></div></div><div class="lg:w-1/2"><div class="w-full aspect-video rounded-2xl bg-gradient-to-br from-blue-900 to-indigo-900 flex items-center justify-center"><HardDrive class="w-24 h-24 text-white/40"/></div></div></div></div></section>
    <section class="py-16 bg-gray-50"><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div class="flex flex-col lg:flex-row gap-12 items-center"><div class="lg:w-1/2"><h2 class="text-2xl font-bold text-gray-800 mb-8">{{ technology.outputTitle }}</h2><div class="space-y-4 text-gray-600 leading-relaxed"><p v-for="item in technology.output" :key="item">{{ item }}</p></div></div><div class="lg:w-1/2"><div class="w-full aspect-video rounded-2xl bg-gradient-to-br from-blue-600 to-blue-800 flex items-center justify-center"><SlidersHorizontal class="w-24 h-24 text-white/40"/></div></div></div></div></section>
  </main><Footer/></div>
</template>
