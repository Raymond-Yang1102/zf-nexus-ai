<template>
  <div class="min-h-screen bg-white">
    ...
    <main class="pt-16">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div v-for="(item, index) in companyData.business.items" :key="index"
              class="p-8 rounded-2xl bg-white border border-gray-100 hover:border-blue-200 hover:shadow-lg transition-all duration-300 text-center">
              <div
                class="w-14 h-14 rounded-xl bg-gradient-to-br from-green-100 to-green-200 flex items-center justify-center mb-6 mx-auto">
      ...
      <section class="py-16 bg-white">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 class="text-2xl font-bold text-gray-800 mb-8 text-center">{{ companyData.companyInfo.title }}</h2>
          <div class="overflow-x-auto">
            <table class="w-full border-collapse">
              <tbody>
                <tr v-for="(row, index) in companyData.companyInfo.table" :key="index" class="border-b border-gray-200">
                  <th class="text-center py-4 px-6 font-semibold text-gray-800 w-1/3">{{ row.label }}</th>
                  <td class="text-center py-4 px-6 text-gray-600">{{ row.value }}</td>
                </tr>
              </tbody>
            </table>
          </div>
